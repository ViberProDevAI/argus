...
struct PositionPlan: Codable, Identifiable {
    ...
    // FAZE 2.2: Trailing stop tracking
    var highestPrice: Double = 0.0
    var trailingStopPrice: Double? = nil

    /// Plan oluşturulduğu anda aktif makro rejim etiketi (örn. "BULLISH",
    /// "BEARISH"). Nil: rejim bilgisi yoktu / eski plan.
    var createdAtRegime: String?

    /// Rejim değiştiğinden beri plan gözden geçirilmedi. UI bu bayrağa göre
    /// "Plan güncel değil" rozeti gösterebilir; Vortex yeniden değerlendirmeyi
    /// tetikleyebilir. Nil = hiç flag yakılmadı (eski plan).
