...
struct ExecutionModel: Codable, Sendable {
    ...
    /// Sembol tipine göre doğru preset'i döner.
    /// BIST (`.IS` / `.BIST` suffix) → `retailTR` (komisyon %0.15)
    /// Diğer → `retailUS` (Alpaca-tarzı komisyonsuz).
    static func forSymbol(_ symbol: String) -> ExecutionModel {
        let upper = symbol.uppercased()
        if upper.hasSuffix(".IS") || upper.hasSuffix(".BIST") {
            return .retailTR
        }
        return .retailUS
    }
