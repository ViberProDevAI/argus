...
struct OrionBrain {
    ...
    func analyze(symbol: String, candles: [Candle], spyCandles: [Candle]?) -> OrionScoreResult? {
        ...
        // 1. Get Weights (Chiron > Deep Tune > Default, rejim-aware)
        let weights = getWeights(symbol: symbol, candles: sorted)
    ...
    // MARK: - Helpers
    private func getWeights(symbol: String, candles: [Candle] = []) -> (structure: Double, trend: Double, momentum: Double, pattern: Double, volatility: Double) {
        // Base weights — öğrenilmiş > configured > default
        let base: (structure: Double, trend: Double, momentum: Double, pattern: Double, volatility: Double)
        ...
        // Rejim-aware düzeltme: ADX > 25 trending; ADX < 15 choppy; arası nötr.
        // Eski sürüm sabit ağırlık kullanıyordu → ranging piyasada momentum+trend fazla
        // ağırlıklıydı, whipsaw üretiyordu. Şimdi yatay piyasada structure+pattern,
        // trending piyasada trend+momentum öne geçiyor.
        let adjusted: (structure: Double, trend: Double, momentum: Double, pattern: Double, volatility: Double)
        if candles.count >= 30 {
            let adx = IndicatorService.lastADX(candles: candles) ?? 20
            if adx > 25 {
                // Trending: trend + momentum artır, structure + pattern azalt
                adjusted = (
                    structure: base.structure * 0.80,
                    trend:     base.trend     * 1.25,
                    momentum:  base.momentum  * 1.20,
                    pattern:   base.pattern   * 0.85,
                    volatility: base.volatility
                )
            } else if adx < 15 {
                // Choppy/ranging: structure + pattern artır, trend + momentum azalt
                adjusted = (
                    structure: base.structure * 1.30,
                    trend:     base.trend     * 0.70,
                    momentum:  base.momentum  * 0.75,
                    pattern:   base.pattern   * 1.20,
                    volatility: base.volatility * 1.15
                )
            } else {
                adjusted = base
            }
        ...
        // Normalize — toplam 1.0 olsun (sonra 100 ile çarpılır)
        let sum = adjusted.structure + adjusted.trend + adjusted.momentum + adjusted.pattern + adjusted.volatility
        guard sum > 0 else { return (20, 20, 20, 20, 20) }
        return (
            adjusted.structure  / sum * 100,
            adjusted.trend      / sum * 100,
            adjusted.momentum   / sum * 100,
            adjusted.pattern    / sum * 100,
            adjusted.volatility / sum * 100
        )
    }
