...
class APIService {
    ...
    // ⚠️ Replace with your own API Key if needed
    private var apiKey = "" // Secrets.finnhubKey removed
    ...
    private func fetchRealQuote(symbol: String) async throws -> Quote? {
        ...
        // Y1 fix: default 60s yerine 10s timeout.
        var request = URLRequest(url: url, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: 10)
        request.setValue("Mozilla/5.0", forHTTPHeaderField: "User-Agent")

        let (data, response) = try await URLSession.shared.data(for: request)

        // Y2 fix: HTTP status code explicit check. Yahoo 429/401 JSON'ı boş result
        // olarak decode olabilir; başarılı görünüp sessizce "veri yok" dönmesin.
        if let http = response as? HTTPURLResponse, !(200...299).contains(http.statusCode) {
            throw URLError(.badServerResponse, userInfo: [NSLocalizedDescriptionKey: "HTTP \(http.statusCode) from Yahoo v7"])
        }

        let decoded = try JSONDecoder().decode(YahooQuoteResponse.self, from: data)

        guard let result = decoded.quoteResponse.result.first else { return nil }
    ...
    private func fetchRealCandles(symbol: String, resolution: String) async throws -> [Candle] {
        ...
        // Y2: 15s timeout + status guard. Yahoo chart endpoint'i rate-limit ederken
        // body'de boş chart.result dönüyor; decode geçiyor ama "[]" caller'a stale'si
        // varmış gibi görünüyor. Status check bu sessiz boş dönüşü hataya çevirir.
        ...
        guard let http = httpResponse as? HTTPURLResponse, http.statusCode == 200 else {
            ...
            throw URLError(.badServerResponse, userInfo: [
                NSLocalizedDescriptionKey: "Yahoo chart HTTP \(code) for \(symbol)"
            ])
        }
