...
final class YahooFinanceProvider: HeimdallProvider {
    ...
    func fetchQuote(symbol: String) async throws -> Quote {
        // Y6: Strict resolver — network'e gitmeden çürük sembolü rate-limit'sizce ret eder.
        // This handles "SILVER" -> "SI=F", "DXY" -> "DX-Y.NYB" etc.
        let ySymbol = try SymbolResolver.shared.resolveStrict(symbol, for: .yahoo)
        ...
        do {
            ...
            q.timestamp = Date() // K4: freshness guard için
            return q
        } catch {
    ...
    func fetchBatchQuotes(symbols: [String]) async throws -> [String: Quote] {
        ...
        // Y6: Strict resolve + batch'ten çürük sembolleri süz. `compactMap` ile
        // invalid olanı atla; iyi semboller tek istekte gider, rate budget boşa gitmez.
        // Original-resolved map'i de burada tutarak 174. satırdaki reverse-lookup için koruyoruz.
        let validPairs: [(original: String, resolved: String)] = symbols.compactMap { orig in
            guard let resolved = try? SymbolResolver.shared.resolveStrict(orig, for: .yahoo) else {
                print("⚠️ Yahoo Batch: Geçersiz sembol atlandı: '\(orig)'")
                return nil
            }
            return (orig, resolved)
        }
        guard !validPairs.isEmpty else { return [:] }
        let yahooSymbols = validPairs.map { $0.resolved }
        ...
        do {
            ...
            for r in results {
                // Y6: Daha önce resolve edilmiş (original, resolved) çiftlerinden reverse lookup.
                // Eskiden her iterasyonda resolve tekrar çağrılıyordu (N², lenient pass-through).
                let originalSymbol = validPairs.first { $0.resolved == r.symbol }?.original ?? r.symbol
                ...
                q.timestamp = Date() // K4: freshness guard
    ...
    private func _fetchFundamentalsInternal(symbol: String, isRetry: Bool) async throws -> FinancialsData {
        ...
        // Y6: Strict — fundamentals hot-path Yahoo'ya çürük sembol göndermemeli.
        let ySymbol = try SymbolResolver.shared.resolveStrict(symbol, for: .yahoo)
        ...
        return FinancialsData(
            ...
            numberOfAnalystOpinions: Int(fin?.numberOfAnalystOpinions?.raw ?? 0)
        )
    }
