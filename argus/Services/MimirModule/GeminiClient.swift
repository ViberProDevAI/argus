...
actor GeminiClient {
    ...
    // 2026-04: preview-05-20 sunset. GA + sürümlü fallback zinciri.
    ...
    func generateContent(prompt: String) async throws -> String {
        // 1. Try Gemini key pool (primary + backup)
        let pool = Secrets.geminiKeyPool
        for (idx, key) in pool.enumerated() {
            do {
                let result = try await callGemini(prompt: prompt, apiKey: key)
                if idx > 0 { print("✅ GeminiClient: Backup key \(idx + 1) başarılı") }
                return result
            } catch {
                print("⚠️ GeminiClient: Gemini key \(idx + 1) başarısız (\(error)). Sıradaki deneniyor...")
            }
        }
