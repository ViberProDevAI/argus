...
actor HealthStore {
    ...
    func reportError(provider: String, error: Error) {
        ...
        if let hErr = error as? HeimdallCoreError {
            // Y3-HOTFIX Phase 2 (2026-04-24): Self-throttle health puanını kirletmesin.
            // HeimdallNetwork lokal rate cap'ini aştığında `code == 1059` sentinel ile
            // `.rateLimited` fırlatır; bu provider arızası değil, kendi frenimiz. Puan
            // düşürmek "provider sağlıksız" sinyali üretir ve AutoPilot/Scout'un sağlık
            // kontrolü `.critical` raporlamaya başlar — self-DoS. Erken çıkış: skor
            // değiştirmeden dön.
            if hErr.code == HeimdallNetwork.localThrottleCode {
                return
            }
