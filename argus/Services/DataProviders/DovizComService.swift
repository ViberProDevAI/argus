...
class DovizComService {
    ...
    private let baseURL = "https://api.doviz.com/v1/"
    private var token: String { APIKeyStore.shared.dovizComToken }
    ...
    func fetchMetalInstitutionRates(asset: String) async throws -> [InstitutionRate] {
        ...
        // Y2: 15s timeout + HTTP status guard. Scraping endpoint 5xx dönerse HTML
        // regex hiçbir şey match etmiyor ve boş liste dönüyor; kullanıcı "kurum yok"
        // sanıyor. Status check sessiz empty tuzağını kapatır.
        let request = URLRequest(url: url, cachePolicy: .reloadIgnoringLocalCacheData, timeoutInterval: 15)
        let (data, response) = try await URLSession.shared.data(for: request)

        guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
            let code = (response as? HTTPURLResponse)?.statusCode ?? -1
            throw DovizComError.networkError(NSError(
                domain: "DovizCom",
                code: code,
                userInfo: [NSLocalizedDescriptionKey: "HTTP \(code) for \(slug)"]
            ))
        }
