...
struct RegimePositionSizer: Sendable {
    ...
    ///
    /// Matematiksel not:
    /// - Eski sürüm `min(macroFactor, regimeFactor)` döndürüyordu; bu iki boyutu birleştirmek yerine
    ///   **en muhafazakâr boyutu** seçiyordu. Örn. macro=0.75 (nötr Aether) + regime=0.20 (riskOff+momentum)
    ///   durumunda yalnızca 0.20 dönerdi; rally başlarken pozisyon aşırı kısılıyordu.
    /// - Yeni sürüm **geometrik/çarpımsal kombinasyon** kullanır: macro * regime.
    ///   Böylece iki boyut birbirini dengeler, rejim taze güncellenmemişse makro hâlâ söz sahibi olur.
    /// - Sonuç her zaman 0.0–1.0 aralığındadır (her iki çarpan da bu aralıkta).
    static func multiplier(aetherScore: Double, regime: MarketRegime, momentumFloor: Double = 0) -> Double {
        ...
        // Çarpımsal birleşim: her iki boyut da etkilidir, tek boyut sinyali silmez.
        // Hem macroFactor hem regimeFactor ∈ [0, 1] olduğundan sonuç da [0, 1] içindedir.
        return macroFactor * regimeFactor
    }
}
