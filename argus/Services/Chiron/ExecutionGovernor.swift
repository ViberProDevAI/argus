...
actor ExecutionGovernor {
    ...
    func review(
        ...
        if !isRiskReducing {
            ...
            // Safe Haven Router — kriz aktifken "contraindicated" sembole girilmez.
            // SafeHavenRouter MainActor'da; tek hop ile iki alan okuyoruz.
            let havenBlock: (blocked: Bool, crisis: String) = await MainActor.run {
                let router = SafeHavenRouter.shared
                guard router.isActive, router.isContraindicated(symbol) else {
                    return (false, "")
                }
                return (true, router.crisisType.rawValue)
            }
            if havenBlock.blocked {
                return .rejected(reason: "Safe Haven aktif (\(havenBlock.crisis)). \(symbol) bu ortamda zayıf.")
            }
        }
